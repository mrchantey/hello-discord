...
mod tests {
    ...
    fn general_api_error_roundtrip() {
        ...
        let deserialized: GeneralApiError = serde_json::from_str(&json).unwrap();
    ...
    fn ratelimited_api_error_roundtrip() {
        ...
        let json = serde_json::to_string(&expected).unwrap();
        let deserialized: RatelimitedApiError = serde_json::from_str(&json).unwrap();
        assert_eq!(expected, deserialized);
    ...
    fn embed_field_serde() {
        ...
        let from_str: MessageApiErrorEmbedField = serde_json::from_str(r#""fields""#).unwrap();
        assert_eq!(from_str, MessageApiErrorEmbedField::Fields);

        let from_str: MessageApiErrorEmbedField = serde_json::from_str(r#""timestamp""#).unwrap();
